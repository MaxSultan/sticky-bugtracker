...
export default class AllBugsChart extends Component {
  ...
  componentDidMount() {
    ...
    .then(res => this.setState({options:{xaxis:{categories: res.data.filter(project => project.status !== "inactive").map(p => p.name)}}}))
    .catch(err => console.log(err))

    Axios.get('/api/projects/getActiveBugNumbers')
